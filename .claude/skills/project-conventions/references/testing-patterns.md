...
2. **Process spawning (`assert_cmd` / `std::process::Command`)** — 26 tests. Miri does not support `fork`/`exec`. Integration tests in `dtvmgr-cli` (cli\_subcommands\_test, cli\_syoboi\_test) use `assert_cmd`, and `dtvmgr-jlse` (command/mod) tests use `std::process::Command` directly.
